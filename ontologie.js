...
function createButtons(parentDiv, items, onClickHandler) {
    ...
    items.forEach(item => {
        ...
        button.classList.toggle("active", item === selectedOrigin || item === selectedType || item === selectedCost);
        button.addEventListener("click", () => {
            const isActive = button.classList.contains("active");
            Array.from(parentDiv.children).forEach(btn => btn.classList.remove("active"));
...
function calculateScore() {
    ...
    team.forEach(champion => {
        for (const [origin, types] of Object.entries(tftOntology.Champions.Origine)) {
            for (const [type, costs] of Object.entries(types)) {
                for (const [cost, champs] of Object.entries(costs)) {
                    if (champs.includes(champion)) {
                        originCounts[origin] = (originCounts[origin] || 0) + 1;
                        typeCounts[type] = (typeCounts[type] || 0) + 1;
                        totalCost += parseInt(cost.split(" ")[0]);
                    }
                }
            }
        }
    ...
    let synergyScore = Object.values(originCounts).reduce((sum, count) => sum + count * count, 0);
    let diversityScore = Object.keys(typeCounts).length * 5; // Bonus pour types variés
    let costScore = totalCost * 2; // Chaque coût ajoute 2 points

    return synergyScore + diversityScore + costScore;
}
